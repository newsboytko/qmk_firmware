...
void override_layer_keymap(int layer_id, uint16_t keymap[MATRIX_ROWS][MATRIX_COLS])
{
    for (int row = 0; row < MATRIX_ROWS; row++)
    {
        for (int col = 0; col < MATRIX_COLS; col++)
        {
            const uint16_t kc = keymap[row][col];

            if (kc != KC_TRNS)
            {
                uint16_t*** writeableKeymaps = (uint16_t***)keymaps;
                writeableKeymaps[layer_id][row][col] = kc;
            }
        }
    }
}
